...
@csrf_exempt
def do_post(request):
  if (request.META['CONTENT_TYPE'] != "application/json") and (request.META['CONTENT_TYPE'] != "application/ld+json"):
  ...
  received = datetime.fromtimestamp(int(time())).strftime('%m-%d-%Y %H:%M:%S %z')
